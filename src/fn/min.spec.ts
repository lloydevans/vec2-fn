...
describe(min.name, () => {
	...
	it("should throw invalid input", () => {
		...
		expect(() => min(null as any, vec1)).toThrowError(ERROR_MESSAGES.INV_V2_DEST);
		expect(() => min(vec1, null as any)).toThrowError(ERROR_MESSAGES.INV_V2_VAL);
		expect(() => min(vec1, vec2, null as any)).toThrowError(ERROR_MESSAGES.INV_V2_VAL);
	});
	...
	it("should check min value", () => {
		...
		expect(min(dest2, vec1, vec2, vec3)).toEqual({ x: -10, y: -1 });
		expect(min(dest3, vec1, vec2, vec3)).toEqual({ x: -10, y: -1 });
	});
