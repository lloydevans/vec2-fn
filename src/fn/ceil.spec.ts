...
describe(ceil.name, () => {
	it("should return inferred types", () => {
		testSingleVecSig(ceil);
	});

	it("should throw invalid input", () => {
		const dest: Vec2 = { x: 0, y: 0 };
		const vec1: Vec2 = { x: -1, y: -1 };
		expect(() => ceil(null as any, dest)).toThrowError(ERROR_MESSAGES.INV_V2);
		expect(() => ceil(vec1, null as any)).toThrowError(ERROR_MESSAGES.INV_V2_DEST);
	});

	it("should ceil values", () => {
		expect(ceil({ x: 0.9, y: 1 + 0.0000001 })).toEqual({ x: 1, y: 2 });
	});
